...
class ReportTestRailTestHealth(Base):
    __table__ = Table("report_testrail_test_health", Base.metadata, autoload_with=pool)  # noqa
...
class ReportJIraQARequestsNewIssueType(Base):
    __table__ = Table(
        "report_jira_qa_requests_new_issue_types", Base.metadata, autoload_with=pool
    )  # noqa
...
class ReportBugzillaQEVerifyCount(Base):
    __table__ = Table(
        "report_bugzilla_qe_needed_count", Base.metadata, autoload_with=pool
    )  # noqa
...
class ReportJiraSoftvisionWorklogs(Base):
    __table__ = Table(
        "report_jira_softvision_worklogs", Base.metadata, autoload_with=pool
    )  # noqa
...
class ReportBugzillaSoftvisionBugs(Base):
    __table__ = Table(
        "report_bugzilla_softvision_bugs", Base.metadata, autoload_with=pool
    )  # noqa
...
class ReportBugzillaQueryByKeyword(Base):
    __table__ = Table(
        "report_bugzilla_query_by_keyword", Base.metadata, autoload_with=pool
    )  # noqa
