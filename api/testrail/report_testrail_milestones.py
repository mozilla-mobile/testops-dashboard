"""Functional TestRail milestones report (restored from monolith).
Fetch -> prepare (DataFrame) -> insert into DB.
"""
import os
...
def _project_id_pairs(db: Database, project: Iterable[str] | str) -> List[Tuple[int, int]]:
    """Return [(projects.id, projects.testrail_project_id), ...] with optional filter."""
    session = db.session
    if isinstance(project, list):
        q = session.query(Projects).filter(Projects.project_name_abbrev.in_(project))
    else:
        q = session.query(Projects).filter(Projects.project_name_abbrev == project)
    results = q.all()
    return [[p.id, p.testrail_project_id] for p in results]


def _fetch_milestones(client: APIClient, testrail_project_id: int) -> list:
    return client.send_get(f"get_milestones/{testrail_project_id}")


def _prepare_milestones_df(payload: list) -> pd.DataFrame:
    if not payload:
        return pd.DataFrame()
    ...
    df = df_all[existing].rename(columns={k: v for k, v in colmap.items() if k in existing})

    # Epoch seconds -> pandas datetime; replace NaN with None for nullable DB cols
    if 'started_on' in df.columns:
        df['started_on'] = pd.to_datetime(df['started_on'], unit='s', errors='coerce')
        df['started_on'] = df['started_on'].replace({np.nan: None})
    if 'completed_on' in df.columns:
        df['completed_on'] = pd.to_datetime(df['completed_on'], unit='s', errors='coerce')
        df['completed_on'] = df['completed_on'].replace({np.nan: None})

    # Derivations from description/name if available
    if 'description' in df.columns:
        df['testing_status'] = df['description'].apply(pl.extract_testing_status)
        df['testing_recommendation'] = df['description'].apply(pl.extract_testing_recommendation)
...
def _insert_milestones(db: Database, projects_id: int, df: pd.DataFrame) -> None:
    if df.empty:
        return
    session = db.session
...
def testrail_milestones_update(project: Iterable[str] | str) -> None:
    ...
    Args:
        project: either a single project abbrev (str) or a list of abbrevs, or 'ALL'.
    """
    db = Database()
    client = _api_client()
    ...
    if isinstance(project, str) and project.upper() == 'ALL':
        # Build list from DB
        proj_filter = [p.project_name_abbrev for p in db.session.query(Projects).all()]

    pairs = _project_id_pairs(db, proj_filter)
    for projects_id, testrail_project_id in pairs:
        payload = _fetch_milestones(client, testrail_project_id)
        df = _prepare_milestones_df(payload)
        _insert_milestones(db, projects_id, df)
